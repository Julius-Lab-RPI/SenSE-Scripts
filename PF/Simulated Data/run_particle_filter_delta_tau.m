function run_particle_filter_delta_tau(subject, run, Q)
...
%% Load subject data.
data_dir = "Data";
file_pref = fullfile(data_dir, "Sim_A", num2str(subject));

load(strcat(file_pref, "_KF"), "phi")
load(file_pref, "t")
load(fullfile(data_dir, "Sim_Light.mat"), "I", "I_dropout")
% I = I_dropout;
load(file_pref, "_LCO", "x")
init = x(1,:);
clear x
...
% there's about 1 day of transients in KF output.
measure_start = 24;
...
% Q = [5e-3, 5e-3, 1e-3, 1e-2, 7e-3];  % Roughening when we resample.
...
%%
save(strcat("Results\Sim_A", num2str(subject), "_PF_Out_Delta_Tau_", num2str(run), ".mat"), ...
